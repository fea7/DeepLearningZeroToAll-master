...
for epoch in range(training_epochs):
    ...
    for i in range(total_batch):
        ...
        for x in range(0, batch_xs.shape[0]):
            # third_image = np.fft.fftshift(np.fft.fft2(batch_xs[x].reshape((28, 28))))
            # for x in range(0, 4):
            #     for y in range(0, 4):
            #         third_image[12 + x, 12 + y] = 0
            # second_image = np.fft.fft2(third_image)
            # second_imageAbs = abs(second_image)
            # batch_xs[x] = second_imageAbs.reshape(784)
            third_image = np.fft.fftshift(np.fft.fft2(batch_xs[x].reshape((28, 28))))
            filterSize = 9
        ...
        # for x in range(0, batch_xs.shape[0]):
        #     batch_xs[x] = np.log(abs(np.fft.fftshift(np.fft.fft2(batch_xs[x].reshape((28, 28))))).reshape(784))
